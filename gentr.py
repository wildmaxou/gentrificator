...
# Données sur le niveaux de vie par arrondissement en 2020
data = {
...
# Page 1: Le Gentrificateur
if page == "Le Gentrificateur":
    arrondissement = st.selectbox('Dans quel arrondissement voulez-vous déménager ?', niveaux_vie["Arrondissement"])
    ...
    with col1:
        if st.button(":red_haired_man:", key='adult1', disabled=st.session_state.nb_adultes >= 1):
            st.session_state.nb_adultes = 1
    with col2:
        if st.button(":red_haired_man::red_haired_man:", key='adult2', disabled=st.session_state.nb_adultes >= 2):
            st.session_state.nb_adultes = 2
    with col3:
        if st.button(":red_haired_man::red_haired_man::red_haired_man:", key='adult3', disabled=st.session_state.nb_adultes >= 3):
            st.session_state.nb_adultes = 3
    with col4:
        if st.button(":red_haired_man::red_haired_man::red_haired_man::red_haired_man:", key='adult4', disabled=st.session_state.nb_adultes >= 4):
            st.session_state.nb_adultes = 4

    st.write("Combien d'enfants de moins de 14 ans avez-vous ? ")
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        if st.button(":baby:", key='baby1', disabled=st.session_state.nb_enfant_moins_14 >= 1):
            st.session_state.nb_enfant_moins_14 = 1
    with col2:
        if st.button(":baby::baby:", key='baby2', disabled=st.session_state.nb_enfant_moins_14 >= 2):
            st.session_state.nb_enfant_moins_14 = 2
    with col3:
        if st.button(":baby::baby::baby:", key='baby3', disabled=st.session_state.nb_enfant_moins_14 >= 3):
            st.session_state.nb_enfant_moins_14 = 3
    with col4:
        if st.button(":baby::baby::baby::baby:", key='baby4', disabled=st.session_state.nb_enfant_moins_14 >= 4):
            st.session_state.nb_enfant_moins_14 = 4

    st.write("Combien d'enfants de plus de 14 ans avez-vous ? ")
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        if st.button(":girl:", key='girl1', disabled=st.session_state.nb_enfant_plus_14 >= 1):
            st.session_state.nb_enfant_plus_14 = 1
    with col2:
        if st.button(":girl::girl:", key='girl2', disabled=st.session_state.nb_enfant_plus_14 >= 2):
            st.session_state.nb_enfant_plus_14 = 2
    with col3:
        if st.button(":girl::girl::girl:", key='girl3', disabled=st.session_state.nb_enfant_plus_14 >= 3):
            st.session_state.nb_enfant_plus_14 = 3
    with col4:
        if st.button(":girl::girl::girl::girl:", key='girl4', disabled=st.session_state.nb_enfant_plus_14 >= 4):
    ...
    if st.button("Es-tu un Gentrificateur ?"):
        ...
        if resultat > niveaux_vie_arrondissement:
            ...
            st.write("Eh non, il faut gagner plus de money 💵")

# Page 2: Estimation prix immobilier
    ...
    arrondissement = st.selectbox('Dans quel arrondissement souhaitez-vous déménager ?', niveaux_vie["Arrondissement"])

    ancien_neuf = st.selectbox('Neuf ou Ancien ?', ("Neuf", "Ancien"))

    appart_maison = st.selectbox('Appartement ou Maison ?', ("Appartement", "Maison"))

    mois = st.selectbox("L'achat se fera en :", ("Janvier", "Fevrier", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"))
